...
// Field concept IDs for occupation fields
const FIELD_CONCEPT_IDS = {
  'it': 'apaJ_2ja_LuF',         // Data/IT
  'data': 'apaJ_2ja_LuF',       // Data/IT
  'teknik': 'Y47T_3YS_MQF',     // Teknik/Ingenjör
  'ingenjör': 'Y47T_3YS_MQF',   // Teknik/Ingenjör
  'ekonomi': 'iVt5_Hua_v89',    // Ekonomi/Administration
  'utbildning': 'gPqD_YQP_LdX', // Utbildning
  'vård': 'oDpj_3CZ_aSZ',       // Hälso- och sjukvård
  'sjukvård': 'oDpj_3CZ_aSZ',   // Hälso- och sjukvård
  'kultur': 'R5nm_8Ef_Ygr',     // Kultur/Media/Design
  'media': 'R5nm_8Ef_Ygr',      // Kultur/Media/Design
  'design': 'R5nm_8Ef_Ygr',     // Kultur/Media/Design
};

// Common Swedish job types
const JOB_TYPES = [
  'sommarjobb',
  'praktik',
  'trainee',
...
/**
 * Format search query to ensure multiple terms are properly combined
...
function formatSearchQuery(query) {
  ...
  // Try to decode the query in case it's URL-encoded
  let decodedQuery;
  try {
  ...
  // If query already has quotes or special operators, leave it as is
  if (decodedQuery.includes('"') || 
      decodedQuery.includes(' AND ') || 
      decodedQuery.includes(' OR ') || 
      decodedQuery.includes('-')) {
    return decodedQuery;
  }
  
  // Split into terms and filter out any terms that are too short
  const terms = decodedQuery.trim().split(/\s+/)
    .filter(term => term.length > 1);
  
  // Simply join terms with spaces - no AND operators
  return terms.join(' ');
}

/**
 * Detect if query contains a job type + field combination pattern
 * @param {string} query - Search query to analyze
 * @returns {Object|null} - Detected pattern or null if no pattern found
 */
function detectSearchPattern(query) {
  ...
  // Need at least 2 words to detect a pattern
  if (words.length < 2) return null;
  
  // Try to find a job type and field in any order
  let jobTypeIndex = -1;
  let fieldIndex = -1;
  
  // First, check if any word is a job type
  for (let i = 0; i < words.length; i++) {
    if (JOB_TYPES.includes(words[i])) {
      jobTypeIndex = i;
      break;
    }
  }
  
  // Then check if any word is a known field
  for (let i = 0; i < words.length; i++) {
    if (i !== jobTypeIndex && FIELD_CONCEPT_IDS[words[i]]) {
      fieldIndex = i;
      break;
    }
  }
  
  // If we found both a job type and field
  if (jobTypeIndex >= 0 && fieldIndex >= 0) {
    return {
      jobType: words[jobTypeIndex],
      field: words[fieldIndex],
      fieldConceptId: FIELD_CONCEPT_IDS[words[fieldIndex]],
      pattern: 'jobtype_field',
      originalQuery: normalizedQuery
    };
  }
  
  // If we only found a job type
  if (jobTypeIndex >= 0) {
    return {
      jobType: words[jobTypeIndex],
      pattern: 'jobtype_only',
      originalQuery: normalizedQuery
    };
  }
  
  // If we only found a field
  if (fieldIndex >= 0) {
    return {
      field: words[fieldIndex],
      fieldConceptId: FIELD_CONCEPT_IDS[words[fieldIndex]],
...
/**
 * Build API URL with properly encoded parameters while preserving search operators
 * @param {string} baseUrl - The base API URL
 * @param {string} endpoint - API endpoint (e.g., 'search' or 'joblinks')
 * @param {string} query - Search query
 * @param {URLSearchParams} params - Other URL parameters
 * @returns {string} - Complete URL with encoded parameters
 */
function buildApiUrl(baseUrl, endpoint, query, params) {
    // Start with the base URL and endpoint
    let url = `${baseUrl}/${endpoint}?`;
    
    // Add the query parameter first, if it exists, with minimal encoding
    // We want to preserve AND/OR operators and quotes
    if (query?.trim()) {
        // Replace spaces with + only when not within quotes
        const encodedQuery = query.trim().replace(/("[^"]*")|(\s+)/g, (match, quoted, space) => {
            if (quoted) return quoted; // Keep quoted strings as is
            if (space) return '+'; // Replace spaces with +
            return match;
        });
        url += `q=${encodedQuery}&`;
    }
...
export async function searchAllJobs({
  ...
  try {
    // Format the search query to properly combine multiple terms
    const formattedQuery = formatSearchQuery(q);
    ...
    if (useApis === 'jobad' || useApis === 'both') {
      apiCalls.push(searchJobAdAPI({
        q: formattedQuery, occupationField, occupationGroup, municipality, region, country, 
...
/**
 * Enhanced job search with pattern detection for smarter results
 * @param {Object} options Search options
 * @returns {Promise<Object>} Combined search results
 */
export async function enhancedJobSearch(options = {}) {
  try {
    const { q = '', ...otherOptions } = options;
    
    // If no query or it's empty, use regular search
    if (!q?.trim()) {
      return searchAllJobs(options);
    }
    
    console.log('Original search query:', q);
    
    // Detect search pattern
    const pattern = detectSearchPattern(q);
    console.log('Detected pattern:', pattern);
    
    // No pattern detected - use regular search with improved formatting
    if (!pattern) {
      console.log('No pattern detected, using formatted query');
      return searchAllJobs({
        ...options,
        // Use original query but formatted for better results
        q: formatSearchQuery(q)
      });
    }
    ...
    switch (pattern.pattern) {
      case 'jobtype_field':
        // For job type + field pattern, use both the job type as a search term
        // AND the occupation field filter for more targeted results
        searchOptions = {
          ...otherOptions,
          q: pattern.originalQuery, // Keep the full original query
          occupationField: pattern.fieldConceptId // And filter by field
        };
        console.log('Job type + field search options:', searchOptions);
        break;
        
      case 'field_only':
        // For field only pattern, use original query plus field filtering
        searchOptions = {
          ...otherOptions,
          q: pattern.originalQuery, // Keep original query
          occupationField: pattern.fieldConceptId
        };
        console.log('Field only search options:', searchOptions);
        break;
        
      case 'jobtype_only':
        // For job type only pattern, use original query
        searchOptions = {
          ...otherOptions,
          q: pattern.originalQuery
        };
        console.log('Job type only search options:', searchOptions);
        break;
        
      default:
        // Fall back to regular search with original query
        searchOptions = options;
        console.log('Default search options:', searchOptions);
    }
    
    return searchAllJobs(searchOptions);
  } catch (error) {
...
/**
 * Search for jobs in a specific field with a given job type
 * @param {string} jobType - Type of job (e.g., sommarjobb, praktik)
 * @param {string} field - Field to search in (e.g., it, data)
 * @returns {Promise<Object>} Search results
 */
export async function searchSpecificFieldJobs(jobType, field) {
  if (!jobType || !field) {
    return {
      hits: [],
      total: 0,
      error: 'Both jobType and field are required'
    };
  }

  const conceptId = FIELD_CONCEPT_IDS[field.toLowerCase()];

  // If we have a concept ID for the field, use occupation-field filtering
  if (conceptId) {
    return searchAllJobs({
      q: jobType,
      occupationField: conceptId,
      limit: DEFAULT_LIMIT,
      sort: 'pubdate-desc'
    });
  }

  // If no concept ID, fall back to regular query
  return searchAllJobs({
    q: `${jobType} ${field}`,
    limit: DEFAULT_LIMIT,
    sort: 'pubdate-desc'
